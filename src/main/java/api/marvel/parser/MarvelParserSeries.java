...
@Log4j2
@Component
public class MarvelParserSeries {
    ...
    public void parse() throws ParseException {
        ...
        for (Character character : characters) {
            ...
            Request request = new Request.Builder()
                    .url(MARVEL_HOST + MARVEL_API + "characters/"+idCharacter+"/series?" + MARVEL_APIKEY + MARVEL_PARAM_LIMIT_100)
                    .method("GET", null)
                    .build();
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                log.info("Series Found:" + resultsJson.length());

                for (int i = 0; i < resultsJson.length(); i++) {
                    JSONObject json = resultsJson.getJSONObject(i);

                    Series series = getSeriesFromJson(json);
                    series.setIdCharacter(idCharacter);
                    seriesList.add(series);

                    log.info("Insert Comic:" + series.getId() + " - Id Character:" + series.getIdCharacter());
                    log.info("Title:" + series.getTitle());
    ...
    private Series getSeriesFromJson(JSONObject json) throws ParseException {
        ...
        int startYear = (int) eventsData.get("startYear");
        int endYear = (int) eventsData.get("endYear");
        String description = (String) eventsData.get("description");
        String resourceURI = (String) eventsData.get("resourceURI");
        String rating = (String) eventsData.get("rating");
        ...
        series.setModified(DateFromDateIso.getDateFromString(modified));
